 ...
 * Returns error pages.
 */
function at_commerce_preprocess_page(&$vars) {   
  $status = drupal_get_http_header("status");  
  if($status == "404 Not Found") {      
        $vars['theme_hook_suggestions'][] = 'page__error';
        $vars['error_code'] = "<blockquote p><h1>Ошибка 404</h1></blockquote p>";
        $error_content = "<blockquote p>Запрашиваемая страница отсутствует на нашем сайте.";
        $error_content .= "Возможно, вы ошиблись при наборе адреса или перешли по неверной ссылке.</blockquote p>";
        $error_content .= "<blockquote p><dl><li>Проверьте правильность написания адреса</li>";
        $error_content .= "<li>Воспользуйтесь главной страницей ";
        $error_content .= l('Asha', '', array('attributes' => array('title' => t('Home')), 'html' => TRUE));
        $error_content .= "</li></ul></blockquote p>";
        $vars['error_content'] = $error_content;
  }
  
    if($status == "403 Forbidden") {      
        $vars['theme_hook_suggestions'][] = 'page__error';
        $vars['error_code'] = "<blockquote p><h1>Ошибка 403</h1></blockquote p>";
        $error_content = "<blockquote p>В доступе к запрашиваемой странице отказано.";
        $error_content .= "Возможно, вы ошиблись при наборе адреса, перешли по неверной ссылке или для доступа необходимо зарегистрироваться.</blockquote p>";
        $error_content .= "<blockquote p><dl><li>Проверьте правильность написания адреса</li>";
        $error_content .= "<li>Воспользуйтесь главной страницей ";
        $error_content .= l('Asha', '', array('attributes' => array('title' => t('Home')), 'html' => TRUE));
        $error_content .= "</li><li>";
        $error_content .= l('Зарегистрируйтесь', 'user/register', array('attributes' => array('title' => t('Login')), 'html' => TRUE));
        $error_content .= " на нашем сайте или ";
        $error_content .= l('войдите', 'user/login', array('attributes' => array('title' => t('Login')), 'html' => TRUE));
        $error_content .= "</li></ul></blockquote p>";
        $vars['error_content'] = $error_content;
  }
}
